...
export type RawCandidate = { gtin: string; title: string; brand?: string; quantity?: string; asin?: string; evidence: Evidence; linkedByModel?: string; linkedByName?: boolean; supportingEvidence?: Evidence };
...
export function rankCandidates(query: ProductQuery, raw: RawCandidate[]): Result[] {
  ...
  return [...grouped].map(([gtin,items])=>{
    ...
    if(items.some(x=>x.linkedByName)) reasons.push('Corrispondenza indiretta per nome e formato: verifica il prodotto');
    ...
    let score=.12 + (asinMatched ? .52 : 0) + (items.some(x=>x.linkedByModel) ? .32 : 0) + (items.some(x=>x.linkedByName) ? .28 : 0) + (sim>=.65 ? .30 : sim>=.35 ? .13 : 0) + (providers.size>1 ? .25 : 0) + (expected.length&&!mismatch&&actual.length ? .13 : 0) - (mismatch ? .45 : 0);
